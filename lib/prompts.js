...
async function updateEmployeeManager() {
    sqlQry = "SELECT CONCAT(last_name, \", \" , first_name) AS name, manager_id AS value FROM employee"
    const emps = await connection.query(sqlQry);

    data = await inquirer.prompt(prompts.updateRole(emps))

    sqlQry = "UPDATE employee SET manager_id = ? WHERE id = ?"
    sqlData = [data.manager_id_id, data.employee_id];
    await connection.query(sqlQry, sqlData)
    init();
}

async function deleteDepartment() {
    sqlQry = "SELECT id, name FROM department"
    const deps = await connection.query(sqlQry);

    const data = await inquirer.prompt(prompts.deleteDepartment(deps));

    sqlQry = "DELETE FROM department WHERE name = ?"
    sqlData = [...Object.keys(data), ...Object.values(data)]
